...
void CNTWriteBasicCommand::Execute() {
	...
	else if (nChar == 0x0A || nChar == VK_RETURN) {
		if (column < this->textEditingForm->current->GetLength()) {
...
void CNTDeleteSelectionBasicCommand::Execute() {
	...
	while (i <= end) {
		...
		if (j >= line->GetLength() && i < end) {
			...
			delete this->textEditingForm->note->GetAt(i + 1);
...
void CNTDeleteSelectionCommand::Unexecute() {
	...
	GlyphFactory glyphFactory;
	GlyphScanner glyphScanner(this->selecteds);
	while (glyphScanner.IsEnd() == false) {
		string token = glyphScanner.GetToken();
...
void CNTPasteCommand::Execute() {
	...
	GlyphScanner glyphScanner(this->pasteds);
	GlyphFactory glyphFactory;
	...
	while (j < copiedLine->GetLength()) {
		this->textEditingForm->current->Add(copiedLine->GetAt(j)->Clone());
		j++;
	}
	//복사한 노트의 줄 수만큼 반복한다.
	while (i < glyphClipBoard->GetLength()) {
		//복사한 노트의 현재 줄을 가져오다.
		copiedLine = glyphClipBoard->GetAt(i)->Clone();
