...
TextEditingForm::TextEditingForm(COLORREF backgroundColor, BOOL isShowScroll) {
...
void TextEditingForm::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) {
	if (nChar >= 32 || nChar == VK_TAB || nChar == 0x0A/*LineFeed*/ || nChar == VK_RETURN) {
...
void TextEditingForm::OnLButtonUp(UINT nFlag, CPoint point) {
	this->SetFocus();
}
